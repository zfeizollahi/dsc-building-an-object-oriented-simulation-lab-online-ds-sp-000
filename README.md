...
```
    ...
    /anaconda3/envs/learn-env/lib/python3.6/site-packages/tqdm/autonotebook/__init__.py:14: TqdmExperimentalWarning: Using `tqdm.autonotebook.tqdm` in notebook mode. Use `tqdm.tqdm` instead to force console mode (e.g. in jupyter console)
...
def run(self):
    ...
    for _ in tqdm(range(self.total_time_steps)):
        ...
        print("Beginning Time Step {}".format(_))
...
```python
#population_size, disease_name, r0, mortality_rate,  total_time_steps, pct_vaccinated, num_initial_infected)
sim = Simulation(2000, 'Ebola', 2, 0.5, 20, 0.85, 50)
```
    ...
    Total Number of Vaccinated People: 1660
    Total Number of Immune: 1660
    Current Infected: 50
    Deaths So Far: 0
